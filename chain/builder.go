...
func BuildBlock(vm VM, preferred ids.ID) (snowman.Block, error) {
	...
	for mempool.Len() > 0 {
		...
		// TODO from a magic list of compute clients we have (how do we get that? ) kick out GenerateTX requests to the computeclients
		// timeout of 90 seconds to get a valid response, otherwise mark them to be slashed
		// how to kick out a generatetx request?
