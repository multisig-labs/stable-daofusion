...
// "submit" means "add to mempool" (adding to mempool also signals to engine that there are pending txs, which kicks off consensus / proposal flow)
// can either submit via direct RPC call [IssueTx and IssueRawTx] or from a AppGossip channel
...
func (vm *VM) submit(tx *chain.Transaction, db database.Database, blkTime int64, ctx *chain.Context) error {
	...
	// dummy db
	// verify that the transaction is valid in below function
	// parameters / prompt / etc are valid
